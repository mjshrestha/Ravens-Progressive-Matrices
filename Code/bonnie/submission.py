...
class Submission(object):
  ...
  def __init__(self, gtcode, quiz_name, 
    ...
    self.hoth_url = "https://hoth.udacity.com"
    ...
    try:
      ...
      elif self.r.status_code in [404,429,500]:
  ...
  def _bonnie_login(self):
    try:
      if self.provider == 'udacity':
        ...
        data = {'email' : email, 'password' : password, "next": self.bonnie_url + "/auth/udacity/callback"}

        #Logging into udacity
        r = self.s.post(self.hoth_url + '/v2/authenticate', 
                        data=data, 
        ...
        if not r.url.startswith(self.bonnie_url):
          raise ValueError("Username and password failed (Do you use two-factor?)")

      elif self.provider == 'developer':
        print("Developer Login required.")
        username = input('Username :')
        r = self.s.post(self.bonnie_url + '/auth/developer/callback', 
  ...
  def _mkzip(self):
    ...
    if os.path.commonprefix([dirname] + filenames) != dirname:
      raise ValueError("Submitted files must in subdirectories of %s." % (dirname or "./"))

    with zipfile.ZipFile(Submission.submission_filename,'w') as z:
      for f in self.filenames:
        zpath = os.path.relpath(f, dirname)

        head = zpath
        while True:
          head, tail = os.path.split(head)
          if tail == "..":
            raise ValueError(".. is part of the path for " + zpath)
          if head == "":
            break
