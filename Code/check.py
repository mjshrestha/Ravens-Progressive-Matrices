...
def solve3by3(problem):
    ...
    max_hist = 0
    ######################################
    # if problem.name =='Basic Problem C-08' :#or problem.name =='Basic Problem C-05' :
    #     print()
    # else:
    #     return -1
    ######################################
    ...
    #     temp_hist = ques_img[nos].histogram()
    #     hist_img.append(temp_hist)
    #     if temp_hist[0] < min_hist:  # find the min number of black pixels in the questions
    #         min_hist = temp_hist[0]
    #     if temp_hist[0] > max_hist:  # find the max number of black pixels in the questions
    #         max_hist = temp_hist[0]
    #
    # if (min_hist / max_hist) < 0.15:  # check histogram pct
    #     ACCURACY_THRES = 95

    ans = check_similars(ques_img, ans_img, ACCURACY_THRES)
    if ans[0]:
        return best_ans
    # check_increasing_size(ques_img[2], ques_img[5], ques_img[6], ques_img[7])# if increasing C and F and increasing pixels G and H then ans is also increasing
    # res = check_C08types(ACCURACY_THRES)
    # if res[0]:
    #     return res[1]
    check_increasing_pixelTypes(ACCURACY_THRES)
    if best_ans != -1:
        return best_ans
    res = check_vert_halfis_mirror(ACCURACY_THRES)
    if res[0]:
        return res[1]
    res = check_horiz_halfis_mirror(ACCURACY_THRES)
    if res[0]:
        return res[1]
    # check_ver_halfissame(ACCURACY_THRES)
...
def check_similarity(img1_q, img2_q, img3_q, test_img, ACCURACY_THRES):
    ...
    # temp1 = ques_img[3].histogram()
    # temp2 = ques_img[4].histogram()
    similar = False

    # if pct_diff <= ACCURACY_THRES and pct_diff_b<= ACCURACY_THRES:
...
# def check_substract():
#     thres=0.1
#     B_sub_CA= find_substract(ques_img[2], ques_img[0])
#     E_sub_FD= find_substract(img1, img2)
...
def check_increasing_rows():
    ...
    # dark_d = ques_img[3].histogram()[0]  # D
    # dark_e = ques_img[4].histogram()[0]  # E
    # dark_f = ques_img[5].histogram()[0]  # F
    if (dark_pA == 0):
        dark_pA = 1
            ...
            for nos in range(len(ans_img)):
                ...
                if sel_dark_pix < dark_pixels_F and sel_dark_pix < dark_pixels_H:
                    ...
                    if abs(
                            ratio_ans_F - ratio_ans_H) < thres:  # if the ratio of option to F and H is the same then it could be the ans
                        if diff_ratio < diff_val:
                            if (diff_ratio != 0) and ratio_ans_H >= thres:
                                diff_val = diff_ratio
                                sel_opt = nos + 1
                                select = True
...
def check_vert_halfis_mirror(ACCURACY_THRES):  # checking if the left half of the image is flipped, EG C09
...
def check_vert(img1, img2, img3, ACCURACY_THRES):
    ...
    return found_ans, best_ans_here
...
def check_horiz_halfis_mirror(ACCURACY_THRES):  # checking if the top half of the image is mirror or flip
...
def check_horiz(img1, img2, img3, ACCURACY_THRES):
    ...
    if res_top_mirror[0] and res_bottom_mirror[0]:
        ...
        img_C = img3
        [len_C, height_C] = img_C.size
        im_top_C = img_C.crop((0, 0, height_C / 2, len_C))  # left image from the middle
        im_bottom_C = img_C.crop((height_C / 2 + 1, 0, height_C, len_C))  # right image from the middle
